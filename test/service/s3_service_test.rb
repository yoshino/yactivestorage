...
if SERVICE_CONFIGURATIONS[:s3]
  class ActiveStorage::Service::S3ServiceTest < ActiveSupport::TestCase
    ...
    test "direct upload" do
      # FIXME: This test is failing because of mismatched request signature, but it works in browser.
      skip

      begin
        key = SecureRandom.base(24)
        data = "Somthing else entirely!"
    ...
    test "signed URL generation" do
      assert_match /rails-activestorage\.s3\.amazonaws\.com.*response-content-disposition=inline.*avatar\.png/,
        @service.url(FIXTURE_KEY, expires_in: 5.minutes, disposition: :inline, filename: "avatar.png") # 5.minutesのところでエラー（railsのメソッドが使えない＋s3のバージョン変更）
    end
  end
else
