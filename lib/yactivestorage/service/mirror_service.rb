...
class Yactivestorage::Service::MirrorService < Yactivestorage::Service
  ...
  def self.build(service_config, all_configurations) #:nodoc:
    primary = Yactivestorage::Service.configure(service_config.fetch(:primary), all_configurations)
     mirrors = service_config.fetch(:mirrors).collect do |service_name|
      Yactivestorage::Service.configure(service_name.to_sym, all_configurations)
    end
